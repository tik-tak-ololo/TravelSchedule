...
Перед первой сборкой создайте локальный файл с API-ключом:

```sh
cp Configurations/Secrets.xcconfig.example Configurations/Secrets.xcconfig
```

Затем замените значение `YOUR_API_KEY` в `Secrets.xcconfig` на ключ API Яндекс.Расписаний. Локальный файл с ключом исключён из Git.
